...
class PostController extends Controller
{
    ...
    public  function edit($postId)
    {
        ...
        $tags = Tag::where('is_category', 0)->get();
        $categoryTags = Tag::where('is_category', 1)->get();
        $news_tags = $post->tags->pluck('id')->toArray();

        return  view("news.edit", compact('post', 'tags', 'categoryTags', 'news_tags'));
    }

    public function update(Request $request, $newsId)
    {
        $news = Post::where('news.id', $newsId)
            ->join('users', 'news.user_id', '=', 'users.id')
    ...
    public  function delete($postId)
    {
        ...
        $news = Post::where('id', $postId)
            ...
            DB::table('news')->where('id', $postId)->delete();
