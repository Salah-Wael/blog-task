...
class PostController extends Controller
{
    ...
    public function index()
    {
        $myPosts = Post::where('user_id', Auth::user()->id)
                        ...
                        ->get();
        if (!is_null($myPosts)) {
            return ApiResponse::sendResponse(200, null, new PostResource($myPosts));
        } else {
    ...
    public  function softDelete($postId)
    {
        ...
        if ($post) {
            File::delete(public_path('assets/img/posts/') . $post->cover_image);
            DB::table('posts')->where('id', $postId)->delete();// Soft delete the post
    ...
    public function viewDeleted()
    {
        ...
        return ApiResponse::sendResponse(200, "Deleted posts retrieved successfully", $deletedPosts);
    }
    ...
    public function restore($postId)
    {
        ...
        if ($post) {
            $post->restore(); // Restore the soft-deleted post

            return ApiResponse::sendResponse(200, "Your post has been restored successfully", []);
        }

        return ApiResponse::sendResponse(403, "Unauthorized or post not found", []);
    }
